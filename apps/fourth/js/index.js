...
// play with these
var settings = {
  colorChangeSpeedFactor: .5, // how fast does the color changes? Lower is slower, exponential
  trailLength: 500, // how long is the trail?
  diameter: 24, // how wide is the spray mark?
  fadeStart: .6 // where does the trail start fading out? percentage along itself
};
...
function frame(time) {
  ...
  cursor.style.boxShadow = cursorTrail.map(function (pos, i) {
    ...
    var age = (settings.trailLength - i) / settings.trailLength;
    var fadeOut = age < settings.fadeStart ? 0 : Math.pow(4 * (age - settings.fadeStart), 2);
    var color = 'hsla(' + pos.hue + ', 50%, 50%, ' + (1 - fadeOut) + ')';
